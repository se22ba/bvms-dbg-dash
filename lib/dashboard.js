...
function extractKeyValueEntries(nodes, start, end) {
 ...
 const DEVICE_METRIC_ALIASES = {
  totalChannels: [
    "canales totales","total canales","numero de dispositivos","número de dispositivos",
    "total de dispositivos","total channels","number of devices","total devices"
  ],
  offlineChannels: [
    "canales fuera de linea","canales fuera de línea","dispositivos fuera de linea","dispositivos fuera de línea",
    "canales offline","offline channels","offline devices","offline"
  ],
  activeRecordings: [
    "grabaciones activas","grabacion activa","grabación activa","dispositivos grabando",
    "active recordings","recordings active","devices recording"
  ],
  signalLoss: [
    "perdida de senal","perdida de señal","pérdida de señal","signal loss"
  ]
};
...
/* ===== Nuevo: elegir la coincidencia con números alrededor ===== */
function pickSectionIndex(nodes, labels) {
  const normLabels = labels.map(normalizeLabelKey);
  const candidates = [];
  nodes.forEach((n, idx) => {
    if (normLabels.includes(normalizeLabelKey(n.text))) candidates.push(idx);
  });
  if (!candidates.length) return null;

  const scoreAround = (start) => {
    let count = 0;
    const end = Math.min(start + 80, nodes.length);
    for (let i = start + 1; i < end; i++) {
      if (extractNumberFromText(nodes[i].text) != null) count++;
    }
    return count;
  };

  candidates.sort((a, b) => scoreAround(b) - scoreAround(a));
  return candidates[0];
}
...
export function parseBoschDashboard(html, vrmId) {
  ...
  sections.forEach(sec => {
    indexByKey[sec.key] = pickSectionIndex(nodes, sec.labels);
  });

  const result = {
    vrmId,
    devices: { entries: [], map: {}, metrics: {}, metricsText: {} },
    storage: { entries: [], map: {} },
    load:    { entries: [], map: {} }
  };
  ...
  /* ===== Fallback: si no encontró “devices”, rastrillar todo ===== */
  if (!result.devices.entries.length) {
    const entries = extractKeyValueEntries(nodes, 0, nodes.length);
    const map = {};
    entries.forEach(e => {
      const k = normalizeLabelKey(e.label);
      if (k) map[k] = e;
    });
    result.devices = { entries, map, metrics: {}, metricsText: {} };
  }

  // Mapear métricas usando alias
  const metrics = {};
  const metricsText = {};
  ...
  // Sanity server-side
  console.log(`[VRM:${vrmId||"importado"}] devices entries=`, result.devices.entries.length, "metrics=", metrics);

  return result;
}

/* ===== Totales y estados cámaras ===== */
function isFiniteNumber(v){ return typeof v === "number" && Number.isFinite(v); }

export function aggregateDeviceTotals(dashboards) {
  const totals = {};
  Object.keys(DEVICE_METRIC_ALIASES).forEach(key => {
    const values = dashboards.map(d => d?.devices?.metrics?.[key]).filter(isFiniteNumber);
